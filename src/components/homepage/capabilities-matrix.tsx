        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-white/[0.03]">
          {capabilities.slice(0, 6).map((cap, i) => (
            ...
            >
              ...
              >
                <h4 className="text-white/70 text-xl font-light mb-2 group-hover:text-[#a7d26d] transition-colors duration-300">
                  {cap.name}
                </h4>
                {cap.description && (
                  <p className="text-white/25 text-base leading-relaxed line-clamp-2 mb-5">{cap.description}</p>
                )}
                <div className="flex items-center gap-2 text-[#a7d26d] text-base font-mono opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <span>Explorer</span>
